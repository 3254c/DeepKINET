...
class sergio (object):
    ...
    noise_type, decays, splice_ratio, sampling_state = 10, tol = 1e-3,\
    window_length = 100, dt = 0.01, optimize_sampling = False,\
    bifurcation_matrix = None, noise_params_splice = None, noise_type_splice = None, dt_splice = 0.01):
        """
        ...
        if np.isscalar(decays) == 1:
            decays_per_genes = np.repeat(decays, number_genes)
            self.decayVector_ = np.tile(decays_per_genes, (number_bins,1))
        elif np.shape(decays)[0] == number_genes:
            self.decayVector_ = np.tile(decays, (number_bins,1))
        elif np.shape(decays)[0] == number_bins:
        ...
        if self.dyn_:
            ...
            if np.isscalar(splice_ratio) == 1:
                splice_ratio_per_genes = np.repeat(splice_ratio, number_genes)
                self.ratioSp_ = np.tile(splice_ratio_per_genes, (number_bins,1))
            elif np.shape(splice_ratio)[0] == number_genes:
                self.ratioSp_ = np.tile(splice_ratio, (number_bins,1))
            elif np.shape(splice_ratio)[0] == number_bins:
                self.ratioSp_ = splice_ratio
            else:
                print ("Error: expect one splice ratio parameter per gene")
                sys.exit()
        ...
        for g in currGenes:
            if g[0].Type == 'MR':
                ...
                for bIdx, rate in enumerate(allBinRates):
                  g[bIdx].append_Conc(np.true_divide(rate, self.decayVector_[bIdx][g[0].ID]))
            ...
            for binID in range(self.nBins_):
                ...
                for g in currGenes:
                    if g[0].Type == 'MR':
                        ...
                        self.binDict[binID][g[0].ID].set_ss_conc_U(np.true_divide(currRate, self.decayVector_[binID][g[0].ID]))
                        self.binDict[binID][g[0].ID].set_ss_conc_S(self.ratioSp_[binID][g[0].ID] * np.true_divide(currRate, self.decayVector_[binID][g[0].ID]))
                    else:
                        params = self.graph_[g[0].ID]['params']
                        currRate = 0
                        for interTuple in params:
    ...
    def calculate_prod_rate_S_(self, gID, binID, num_c_to_evolve):
        ...
        U = np.array(U)
        return self.decayVector_[binID][gID] * U
    ...
    def dynamics_CLE_simulator_(self, binID):
        ...
        while (resume):
            for gID, g in enumerate(sim_set):
                ...
                decay_S = np.true_divide(self.decayVector_[binID][gID], self.ratioSp_[binID][gID]) * currS
